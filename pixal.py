...
class Pixal:
    ...
    def wheelOfFortune():
        ...
        file2 = open('books.csv', 'r', encoding='utf8')
        books = file2.read().splitlines()
        file3 = open('countries.csv', 'r', encoding='utf8')
        ...
        wheel = ['Bankrupt', 750, 800, 300, 200, 1000, 500, 400, 300, 200, 500, 700, \
        200, 500, 450, 'Lose A Turn', 200, 400, 250, 150, 400, 900, 250, 350]
        playerCount = input("\nWelcome to Wheel of Fortune! Please select the \
...
#Set cash totals to zero for each player
            ...
            while round is True:
                ...
                for name in players:
                    if playerScores[name] > 249:
                        action = input("\nDo you want to 'guess' a consonant, \
                        'buy' a vowel or attempt to 'solve'?:")
                    else:
                        action = input("\nDo you want to 'guess' a consonant or \
                        attempt to 'solve'?:")
                    if action == "guess":
                        ...
                        if spin == "Bankrupt":
                            ...
                            print("\nJust a reminder, the letters we've had so far are:", \
                            *sorted(guesses))
                            print("And here's how the grid is shaping up\n")
                            print(*grid)
                            guess = input("\nOkay player, which consonant do you want to try?:")
                        if guess in answer and guess not in guesses and guess not in vowel and \
                        len(guess) is 1 and guess.isalpha() is True:
                            ...
                            for i in answer:
                                if i == guess:
                                    ...
                                    for i in indices:
                                        grid[i] = guess
                            print(*grid)
                            print("\nGood guess %s, you're now on a total of $" % name, \
                            playerScores[name])
                        #If the letter guessed is not in the word
                            ...
                            print("\nPay attention %s, we've already had that letter! \
                            Next player!" % name)
                        elif guess in vowel:
                            print("\nSorry %s if you want vowels, you'll have to pay \
                            for them!" % name)
                        else:
                            print("\nOh tough break %s, let's move on to the next player" \
                            % name)
                        ...
                        if toBuy in answer and toBuy not in guesses and toBuy in vowel:
                            ...
                            guesses.append(toBuy)
                            print("\nToo bad %s - no %s in this title! On to the next player" \
                            % (name, toBuy))
                    elif action == "solve":
                        solve = input("\nGood luck %s, please enter the answer in the format shown \
                        by the grid - all lowercase:" % name)
                        if solve == answer:
                            for i in grid:
                                if i == "_":
                                    multiplier += 1
                            ...
                            print("\nAnd the winner for the %s round with a score of $%d is %s! \
                            Great job!" % (stage[rounds.index(value)], highest, winner))
                            guess = ""
                            round = False
                        else:
        ...
        totals = sorted(playerScores.values())
        highest = totals.pop(-1)
        print("\nAnd today's winner with a grand total of $%d is %s! Congratulations, we hope to \
        see you all again!" % (highest, winner))
